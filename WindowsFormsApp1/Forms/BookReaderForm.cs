...
namespace WindowsFormsApp1.Forms
{
    public partial class BookReaderForm : Form
    {
        ...
        { 
            ...
            // Lấy độ dài nội dung của chương hiện tại
            int chapterContentLength = 0;
            if (_currentChapterIndex >= 0 && _currentChapterIndex < _chapters.Count)
            {
                chapterContentLength = _chapters[_currentChapterIndex].Content?.Length ?? 0;
            }
            
            // Truyền thêm tổng số chương và độ dài chương để tính tiến độ chính xác
            _readerService.SaveReadingPosition(
                _book.Id, 
                DataManager.Instance.GetCurrentUser(), 
