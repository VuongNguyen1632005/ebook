...
namespace WindowsFormsApp1.Services
{
    public class BookReaderService
    {
        ...
        public void SaveReadingPosition(int bookId, int userId, int chapterNumber, int positionInChapter, int totalChapters = 0, int chapterContentLength = 0)
        {
            try
            {
                using (var conn = DatabaseConnection.Instance.GetConnection())
                {
                    ...
                    // Tính tiến độ chính xác theo công thức:
                    // Progress = (ChapterIndex / TotalChapters) + (PositionInChapter / ChapterLength) * (1 / TotalChapters)
                    // Ví dụ: Chương 5/10, đọc được 50% chương → 50% + 5% = 55%
                    
                    double chapterProgress = 0;
                    if (totalChapters > 0)
                    {
                        // Tiến độ cơ bản từ các chương đã đọc xong
                        double baseProgress = (double)chapterNumber / totalChapters;
                        
                        // Tiến độ bổ sung từ vị trí trong chương hiện tại
                        double inChapterProgress = 0;
                        if (chapterContentLength > 0 && positionInChapter > 0)
                        {
                            // Tỷ lệ đã đọc trong chương hiện tại
                            double positionRatio = Math.Min(1.0, (double)positionInChapter / chapterContentLength);
                            // Quy đổi ra % của toàn bộ sách (mỗi chương chiếm 1/totalChapters)
                            inChapterProgress = positionRatio / totalChapters;
                        }
                        
                        chapterProgress = baseProgress + inChapterProgress;
                    }
                    ...
                    string query = @"
                        ...
                        -- Cập nhật TrangHienTai dựa trên tiến độ đã tính
                        UPDATE Sach 
                        SET TrangHienTai = CASE 
                            WHEN TongSoTrang > 0 THEN CAST(ROUND(@Progress * TongSoTrang, 0) AS INT)
                            ELSE @Ch 
                        END 
                        WHERE MaSach = @Bid";
                    ...
                    using (var cmd = new SqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Progress", chapterProgress);
                        cmd.ExecuteNonQuery();
                    }
                }
            }
